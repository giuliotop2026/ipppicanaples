...
if uploaded_files:
    ...
    for i, file in enumerate(uploaded_files):
        # Sicurezza 1: Usiamo la libreria Image per aprire il file in modo nativo
        img = Image.open(file)
        with cols[i]: 
            # Sicurezza 2: use_container_width al posto del vecchio use_column_width
            st.image(img, caption=f"Scheda #{i+1}", use_container_width=True)
        # Sicurezza 3: "Riavvolgiamo" il file per farlo leggere correttamente a Gemini dopo!
        file.seek(0)
    ...
    if not uploaded_files:
        ...
        with st.status("🕵️ Avvio Algoritmo Moneyball 1.0...", expanded=True) as status:
            try:
                ...
                st.error(f"⚠️ ATTENZIONE: {str(e)}")import time
import streamlit as st
from google import genai
from PIL import Image
import streamlit.components.v1 as components
