...
# --- 1. GRAFICA WESTERN CHIARA (SALOON EL GRANITO) ---
st.markdown("""
    <style>
    .stApp { 
...
# --- 3. CONNESSIONE A GEMINI (IL RINNEGATO) ---
try:
...
st.markdown("### *'Niente blocchi. Niente chiacchiere. Solo polmoni d'acciaio.'*")
...
# --- 5. IL GRILLETTO (PROTOCOLLO GRANITO 3.0) ---
if st.button("🐎 SCATENA IL DUELLO (ANALIZZA)"):
    if not uploaded_files:
        st.warning("EHI COMPADRE, CARICA I MANIFESTI PRIMA DI SPARARE!")
    else:
        with st.spinner("LO SCERIFFO GEMINI STA PRENDENDO LA MIRA... ⏳"):
            try:
                images = [Image.open(f) for f in uploaded_files]

                # PROMPT SPIETATO [cite: 2026-02-25]
                prompt = f"""
                ...
                NON SPIEGARE NULLA. FORNISCI SOLO NOME E NUMERO.
                
                SE TROVI IL SACRO GRAAL:
                '💰 TAGLIA RISCOSSA: PISTOLERO [NUMERO #] - [NOME]'
                'BULLONE SERRATO: [CONFERMA REQUISITI IN UNA RIGA].'
